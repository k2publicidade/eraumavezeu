...
import { MercadoPagoTransparentCheckout } from "@/components/payments/MercadoPagoTransparentCheckout";
...
export default async function PedidoPage({ params }: PedidoPageProps) {
  ...
  const buyerNameParts = (order.guestName || "").trim().split(/\s+/);
  const mercadoPagoPublicKey = process.env.MP_PUBLIC_KEY?.trim().replace(/^['"]|['"]$/g, "") || "";

  const isAwaitingPayment = order.status === "AGUARDANDO_PAGAMENTO";
  const isPaid = order.paymentStatus === "APROVADO";
  const isPending = order.paymentStatus === "PENDENTE" && Boolean(order.paymentId);
  const isFailure = order.paymentStatus === "REJEITADO";
        ...
        <div className="card-premium p-8 md:p-10 text-center">
          ...
          <p className="mt-3 text-dark/70 leading-relaxed">
            {isPaid ? (
              ...
              `Seu pedido foi reservado, ${buyerFirstName}. Escolha PIX ou cartão abaixo para concluir a compra sem sair do site.`
            )}
          </p>
          ...
          {isAwaitingPayment && !isPaid && (
            <div className="mt-8 border-t border-gold/15 pt-8">
              <h2 className="mb-2 font-serif text-2xl text-primary">Como deseja pagar?</h2>
              <p className="mb-6 text-sm text-dark/60">
                Selecione uma opção segura abaixo. Você continuará nesta página.
              </p>
              {mercadoPagoPublicKey ? (
                <MercadoPagoTransparentCheckout
                  ...
                  payer={{
                    ...
                    firstName: buyerNameParts[0] || undefined,
                    lastName: buyerNameParts.slice(1).join(" ") || undefined,
                    cpf: order.guestCpf?.replace(/\D/g, "") || undefined,
                  }}
                  existingPix={order.pixQrCode && order.pixQrCodeBase64 ? {
                ...
                <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">
                  Pagamento temporariamente indisponível. A chave pública do Mercado Pago não está configurada.
                </div>
              )}
            </div>
          )}
