      ...
      <section className="py-16 md:py-24 bg-gradient-to-b from-light to-primary/5">
        <div className="container mx-auto px-4 max-w-3xl text-center">
          ...
          <p className="mt-4 text-lg text-dark/70">
            Não achou sua dúvida? Chama no WhatsApp.
          </p>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="space-y-3">
            {FAQ.map((item, idx) => (
              <details
                key={idx}
                className="group bg-white rounded-2xl border border-primary/10 overflow-hidden"
              >
                <summary className="cursor-pointer px-6 py-4 font-serif text-lg text-dark flex items-center justify-between list-none">
                  <span>{item.q}</span>
                  <span className="text-primary text-2xl group-open:rotate-45 transition-transform">
                    +
                  </span>
                </summary>
                <div className="px-6 pb-5 text-dark/75 leading-relaxed">
                  {item.a}
                </div>
              </details>
            ))}
