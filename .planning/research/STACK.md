# Stack Research — Era Uma Vez Eu

**Domain:** E-commerce BR de livros infantis personalizados com IA (wizard + uploads + MP + Correios)
**Researched:** 2026-04-18
**Confidence:** HIGH (versões verificadas via docs oficiais/npm; gotchas BR verificados em docs Mercado Pago/Melhor Envio)

---

## Executive Summary

Stack **prescritivo** para atender o brief do cliente mantendo aderência à realidade BR (PIX obrigatório, Correios via Melhor Envio, LGPD para fotos de crianças). O cliente fixou **Next.js 14 App Router** como constraint — respeitamos, mas sinalizamos que **Next.js 15.2.4** é o atual estável (março 2026) e seria a escolha padrão para greenfield. Manter 14 é válido (é LTS-like e suportado), mas perdemos Turbopack estável e async Request APIs.

Principais decisões opinativas:
...
npx create-next-app@14 eraumavezeu \
  --typescript --tailwind --eslint --app --src-dir=false --import-alias="@/*"

cd eraumavezeu

# 2. Core: DB + Auth + Validation
npm install @prisma/client @auth/prisma-adapter next-auth@beta zod react-hook-form @hookform/resolvers
...
# 4. Integrações BR
npm install mercadopago                          # ^2.12+
npm install resend @react-email/components       # Resend ^4, RE ^3
npm install uploadthing @uploadthing/react       # ^7

# 5. Media + Flipbook
npm install sharp                                # ^0.33
npm install react-pageflip                       # 2.0.3 — ver "What NOT to Use" para alternativa
...
**Obrigatório** verificar assinatura HMAC-SHA256 em **toda** notificação — caso contrário, atacante pode forjar webhook e marcar pedido como pago sem pagar.

**Algoritmo:**
```
template = `id:${dataId};request-id:${xRequestId};ts:${timestamp};`
hash     = HMAC-SHA256(template, MP_WEBHOOK_SECRET)
valid    = (hash === signatureFromHeader)
```
O header `x-signature` vem no formato `ts=1234567890,v1=hash...`. Partir por vírgula, extrair `ts` e `v1`.

**Gotcha:** `MP_WEBHOOK_SECRET` é **diferente** do `MP_ACCESS_TOKEN` — é gerado no painel MP ao configurar o webhook e **não tem prazo de validade**, mas docs recomendam rotação periódica. Confidence: HIGH (docs oficiais MP).

### 2. Melhor Envio — OAuth2, não Bearer estático
O brief do cliente mostra uso de `Authorization: Bearer ${process.env.MELHOR_ENVIO_TOKEN}` — **isso é simplificação que só funciona com Personal Access Token de conta sandbox pessoal**. Para produção com múltiplos clientes, Melhor Envio exige OAuth2 completo (client_id/client_secret, refresh token, fluxo de autorização). Para e-commerce de loja única (caso Era Uma Vez Eu), Personal Access Token funciona — **documentar isso** para não surpreender em escala.

**Sandbox:** `sandbox.melhorenvio.com.br` com saldo fictício R$ 10.000 — registrar app lá separado da produção. **Credenciais não são portáveis entre ambientes.**

**Serviços (IDs):** `1` = PAC, `2` = SEDEX, `17` = Mini Envios. Mini Envios é o mais barato para livros (pacote até 1kg e 30cm, caso do livro capa dura de 32cm — **verificar se entra; se não, cai para PAC**).

**Atenção unidades:** API trabalha com **centímetros** (dimensions) e **kg** (weight). Enviar em mm ou gramas retorna cotação errada silenciosamente. Confidence: HIGH (docs MelhorEnvio + ajuda oficial).

### 3. Sharp na Vercel Serverless
Sharp precisa do binário linux-x64 musl. Vercel **normalmente resolve automaticamente** via `npm install`, mas em monorepos / custom builds às vezes falha com "module not found". Solução:
```json
...
```
Alternativa: colocar watermark em **route handler na runtime Node.js** (não Edge — Edge não suporta Sharp). Confidence: HIGH (GitHub discussions ativas).
...
Evolution API v2 usa Baileys por baixo. Em 2026, relatos consistentes de que **Baileys não está mais acompanhando mudanças do WhatsApp** — números são derrubados, alterações de protocolo sem patch. Para o MVP é aceitável (cliente já escolheu), mas:
...
Default do Prisma abre conexões diretas (porta 5432). Em serverless, **cada cold start abre conexão nova e não fecha rápido** — esgota pool Supabase em minutos sob tráfego.

**Solução obrigatória:** usar Supavisor transaction pooler:
```env
# .env.production
...
```
`DIRECT_URL` é usada apenas pelo `prisma migrate`. `DATABASE_URL` usa porta 6543 (pooler) + `connection_limit=1`.
...
```
Confidence: HIGH (docs Supabase + Prisma troubleshooting oficial).

---

## Sources

**Context7 / docs oficiais consultados:**
...
*Stack research for: E-commerce BR personalizado (livros infantis com IA)*
*Researched: 2026-04-18*
*Next review: ao final da Fase 4 (integração pagamento+frete) para validar MP webhook + Melhor Envio sandbox → prod*
